#!/usr/bin/env python3
"""Read and validate the RV dashboard's published integration manifest."""
...
PEER_SITE_ID = "rv-dashboard"
PEER_MANIFEST_URL = "https://larry890122.github.io/rv-dashboard/integration-manifest.json"
REQUIRED = {
    "schema_version", "site_id", "production_url", "commit_sha", "built_at",
    "content_as_of", "validation_status", "peer",
}


def validate_peer(data: dict) -> None:
    missing = REQUIRED - data.keys()
    if missing:
        raise ValueError(f"missing fields: {', '.join(sorted(missing))}")
    if data["schema_version"] != 1:
        raise ValueError(f"unsupported schema_version={data['schema_version']!r}")
    if data["site_id"] != PEER_SITE_ID:
